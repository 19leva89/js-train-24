...
function task1() {
	...
	function getData(data, timeout = 2000) {
		...
		return new Promise((resolve, reject) => {
			...
			setTimeout(() => {
				if (data) {
					resolve(data); // Якщо об'єкт не пустий, ми викликаємо resolve з data
				} else {
					reject(new Error("Об'єкт пустий")); // Якщо об'єкт пустий, ми викликаємо reject з новим об'єктом Error("Об'єкт пустий")
				}
			}, timeout);
		})
	}
	...
	getData({ name: "John", age: 30, city: "New York" }, 2000)
		...
		.then((result) => {
			console.log("Успішно отримані дані:", result);
		})
